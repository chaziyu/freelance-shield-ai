...
def build_agent_bundle(model: str | BaseLlm = DEFAULT_MODEL) -> AgentBundle:
    ...
    discussion = Agent(
        ...
        instruction=(
            ...
            "Output must contain exactly these top-level keys and no others:\n"
            "{\n"
            ...
            "}\n"
            "Required rules:\n"
            "- title, missing_fields, and risk_flags are always required.\n"
            "- Never use project_facts.\n"
            "- Use missing_fields, never missing_terms.\n"
            "- Never nest required fields inside another object.\n"
            "- Do not add output_schema.\n"
            "For every non-null field below (scope, deliverables, "
            "fee_amount_minor, currency, deadline, revision_limit, "
            "payment_terms), require exactly this structure:\n"
            "{\n"
            "  \"value\": \"typed value appropriate to the field\",\n"
            "  \"evidence_quote\": \"exact substring copied from the discussion\",\n"
            "  \"confidence\": 0.0\n"
            "}\n"
            "Additional required rules:\n"
            "- deliverables.value must be an array of strings.\n"
            "- fee_amount_minor.value must be an integer in minor units. "
            "Example: RM800 becomes 80000.\n"
            "- currency.value must be \"MYR\" when normalising RM.\n"
            "- revision_limit.value must be an integer.\n"
            "- deadline.value must be a string in ISO date format "
            "(YYYY-MM-DD). If the discussion does not specify a precise "
            "calendar date (e.g. 'Friday'), set deadline to null and "
            "add \"deadline\" to missing_fields.\n"
            "- Use null when a field cannot be supported by an exact quote.\n"
            "- Add the field name to missing_fields rather than guessing.\n"
            "- evidence_quote must be an exact substring from the discussion.\n"
    ...
    safety_audit = Agent(
        ...
        instruction=(
            ...
            "}\n"
            "Rules:\n"
            "- decision must be one of: \"safe_to_show\", \"blocked\", "
            "\"warnings\", \"blocked_reasons\", \"required_human_review\".\n"
            "- Never wrap the response in a nested key like "
            "\"SafetyAuditDecision\".\n"
            "- Do not add extra fields like status or comments.\n"
            "- Output only one JSON object.\n"
            "- No prose, explanation, Markdown, or code fences."
        ),
        tools=[],
